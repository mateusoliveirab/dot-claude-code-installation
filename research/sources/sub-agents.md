...
[Getting started](https://code.claude.com/docs/en/sub-agents/docs/en/overview)[Build with Claude Code](https://code.claude.com/docs/en/sub-agents/docs/en/sub-agents)[Deployment](https://code.claude.com/docs/en/sub-agents/docs/en/third-party-integrations)[Administration](https://code.claude.com/docs/en/sub-agents/docs/en/setup)[Configuration](https://code.claude.com/docs/en/sub-agents/docs/en/settings)[Reference](https://code.claude.com/docs/en/sub-agents/docs/en/cli-reference)[Resources](https://code.claude.com/docs/en/sub-agents/docs/en/legal-and-compliance)

##### Build with Claude Code

* [Create custom subagents](https://code.claude.com/docs/en/sub-agents/docs/en/sub-agents)
* [Run agent teams](https://code.claude.com/docs/en/sub-agents/docs/en/agent-teams)
* [Create plugins](https://code.claude.com/docs/en/sub-agents/docs/en/plugins)
* [Discover and install prebuilt plugins](https://code.claude.com/docs/en/sub-agents/docs/en/discover-plugins)
* [Extend Claude with skills](https://code.claude.com/docs/en/sub-agents/docs/en/skills)
* [Run prompts on a schedule](https://code.claude.com/docs/en/sub-agents/docs/en/scheduled-tasks)
* [Output styles](https://code.claude.com/docs/en/sub-agents/docs/en/output-styles)
* [Automate with hooks](https://code.claude.com/docs/en/sub-agents/docs/en/hooks-guide)
* [Programmatic usage](https://code.claude.com/docs/en/sub-agents/docs/en/headless)
* [Model Context Protocol (MCP)](https://code.claude.com/docs/en/sub-agents/docs/en/mcp)
* [Troubleshooting](https://code.claude.com/docs/en/sub-agents/docs/en/troubleshooting)
...
If you need multiple agents working in parallel and communicating with each other, see [agent teams](https://code.claude.com/docs/en/sub-agents/docs/en/agent-teams) instead. Subagents work within a single session; agent teams coordinate across separate sessions.
...
A research agent used during [plan mode](https://code.claude.com/docs/en/sub-agents/docs/en/common-workflows#use-plan-mode-for-safe-code-analysis) to gather context before presenting a plan.
...
| Plugin’s agents/ directory | Where plugin is enabled | 4 (lowest)  | Installed with [plugins](https://code.claude.com/docs/en/sub-agents/docs/en/plugins) |
...
The `--agents` flag accepts JSON with the same [frontmatter](#supported-frontmatter-fields) fields as file-based subagents: `description`, `prompt`, `tools`, `disallowedTools`, `model`, `permissionMode`, `mcpServers`, `hooks`, `maxTurns`, `skills`, and `memory`. Use `prompt` for the system prompt, equivalent to the markdown body in file-based subagents. **Plugin subagents** come from [plugins](https://code.claude.com/docs/en/sub-agents/docs/en/plugins) you’ve installed. They appear in `/agents` alongside your custom subagents. See the [plugin components reference](https://code.claude.com/docs/en/sub-agents/docs/en/plugins-reference#agents) for details on creating plugin subagents. 
...
| skills          | No       | [Skills](https://code.claude.com/docs/en/sub-agents/docs/en/skills) to load into the subagent’s context at startup. The full skill content is injected, not just made available for invocation. Subagents don’t inherit skills from the parent conversation                                                                   |
| mcpServers      | No       | [MCP servers](https://code.claude.com/docs/en/sub-agents/docs/en/mcp) available to this subagent. Each entry is either a server name referencing an already-configured server (e.g., "slack") or an inline definition with the server name as key and a full [MCP server config](https://code.claude.com/docs/en/sub-agents/docs/en/mcp#configure-mcp-servers) as value |
...
The `model` field controls which [AI model](https://code.claude.com/docs/en/sub-agents/docs/en/model-config) the subagent uses: 
...
Subagents can use any of Claude Code’s [internal tools](https://code.claude.com/docs/en/sub-agents/docs/en/tools-reference). By default, subagents inherit all tools from the main conversation, including MCP tools. To restrict tools, use the `tools` field (allowlist) or `disallowedTools` field (denylist): 
...
Use the `mcpServers` field to give a subagent access to [MCP](https://code.claude.com/docs/en/sub-agents/docs/en/mcp) servers that aren’t available in the main conversation. Inline servers defined here are connected when the subagent starts and disconnected when it finishes. String references share the parent session’s connection. Each entry in the list is either an inline server definition or a string referencing an MCP server already configured in your session: 
...
Claude Code [passes hook input as JSON](https://code.claude.com/docs/en/sub-agents/docs/en/hooks#pretooluse-input) via stdin to hook commands. The validation script reads this JSON, extracts the Bash command, and [exits with code 2](https://code.claude.com/docs/en/sub-agents/docs/en/hooks#exit-code-2-behavior-per-event) to block write operations: 
...
See [Hook input](https://code.claude.com/docs/en/sub-agents/docs/en/hooks#pretooluse-input) for the complete input schema and [exit codes](https://code.claude.com/docs/en/sub-agents/docs/en/hooks#exit-code-output) for how exit codes affect behavior. 
...
You can prevent Claude from using specific subagents by adding them to the `deny` array in your [settings](https://code.claude.com/docs/en/sub-agents/docs/en/settings#permission-settings). Use the format `Agent(subagent-name)` where `subagent-name` matches the subagent’s name field. 
...
See [Permissions documentation](https://code.claude.com/docs/en/sub-agents/docs/en/permissions#tool-specific-permission-rules) for more details on permission rules. 
...
Subagents can define [hooks](https://code.claude.com/docs/en/sub-agents/docs/en/hooks) that run during the subagent’s lifecycle. There are two ways to configure hooks: 
...
Define hooks directly in the subagent’s markdown file. These hooks only run while that specific subagent is active and are cleaned up when it finishes. All [hook events](https://code.claude.com/docs/en/sub-agents/docs/en/hooks#hook-events) are supported. The most common events for subagents are: 
...
See [Hooks](https://code.claude.com/docs/en/sub-agents/docs/en/hooks) for the complete hook configuration format. 
...
* **Foreground subagents** block the main conversation until complete. Permission prompts and clarifying questions (like [AskUserQuestion](https://code.claude.com/docs/en/sub-agents/docs/en/tools-reference)) are passed through to you.
...
To disable all background task functionality, set the `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` environment variable to `1`. See [Environment variables](https://code.claude.com/docs/en/sub-agents/docs/en/env-vars). 
...
For tasks that need sustained parallelism or exceed your context window, [agent teams](https://code.claude.com/docs/en/sub-agents/docs/en/agent-teams) give each worker its own independent context. 
...
Consider [Skills](https://code.claude.com/docs/en/sub-agents/docs/en/skills) instead when you want reusable prompts or workflows that run in the main conversation context rather than isolated subagent context. For a quick question about something already in your conversation, use [/btw](https://code.claude.com/docs/en/sub-agents/docs/en/interactive-mode#side-questions-with-btw) instead of a subagent. It sees your full context but has no tool access, and the answer is discarded rather than added to history. 

Subagents cannot spawn other subagents. If your workflow requires nested delegation, use [Skills](https://code.claude.com/docs/en/sub-agents/docs/en/skills) or [chain subagents](#chain-subagents) from the main conversation.
...
Claude Code [passes hook input as JSON](https://code.claude.com/docs/en/sub-agents/docs/en/hooks#pretooluse-input) via stdin to hook commands. The validation script reads this JSON, extracts the command being executed, and checks it against a list of SQL write operations. If a write operation is detected, the script [exits with code 2](https://code.claude.com/docs/en/sub-agents/docs/en/hooks#exit-code-2-behavior-per-event) to block execution and returns an error message to Claude via stderr. Create the validation script anywhere in your project. The path must match the `command` field in your hook configuration: 
...
* [Distribute subagents with plugins](https://code.claude.com/docs/en/sub-agents/docs/en/plugins) to share subagents across teams or projects
* [Run Claude Code programmatically](https://code.claude.com/docs/en/sub-agents/docs/en/headless) with the Agent SDK for CI/CD and automation
* [Use MCP servers](https://code.claude.com/docs/en/sub-agents/docs/en/mcp) to give subagents access to external tools and data
